...
object Huffman {
  ...
  def convert(tree: CodeTree): CodeTable = {
    ...
    def add(chars: List[Char], bit: Bit, accu: CodeTable): CodeTable = {
      chars map { c => (c, codeBits(accu)(c) :+ bit) }
    }

    def convert0(pTree: CodeTree, accu: CodeTable): CodeTable = {
      pTree match {
        case t: Fork => mergeCodeTables(convert0(t.left, add(chars(t.left), 0, accu)), convert0(t.right, add(chars(t.right), 1, accu)))
        case t: Leaf => accu
      }
    }

    convert0(tree, List[(Char, List[Bit])]())
  }
