...
object Anagrams {
  ...
  /** Converts the word into its character occurrence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = (w.toLowerCase().groupBy(x => x).mapValues(y => y.length) toList).sorted
  ...
  } yield (xi, xj - y.toMap.getOrElse(xi, 0))) sorted
